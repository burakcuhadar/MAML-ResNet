...
class Models:
    ...
    def forward_pretrain(self, inp, weights, is_training=False, reuse=False, scope=''):
        ...
        hidden1 = conv_block(inp, weights['conv1'], weights['b1'], is_training, reuse, scope + '0')
        hidden2 = conv_block(hidden1, weights['conv2'], weights['b2'], is_training, reuse, scope + '1')
        hidden3 = conv_block(hidden2, weights['conv3'], weights['b3'], is_training, reuse, scope + '2')
        hidden4 = conv_block(hidden3, weights['conv4'], weights['b4'], is_training, reuse, scope + '3')
    ...
    def forward(self, inp, weights, step, is_training=False, reuse=False, scope=''):
        ...
        """
        inp = tf.reshape(inp, [-1, self.img_size, self.img_size, self.channels])
        hidden1 = conv_block(inp, weights['conv1'], weights['b1'], is_training, reuse, scope + '0' + str(step))
        hidden2 = conv_block(hidden1, weights['conv2'], weights['b2'], is_training, reuse, scope + '1' + str(step))
        hidden3 = conv_block(hidden2, weights['conv3'], weights['b3'], is_training, reuse, scope + '2' + str(step))
        hidden4 = conv_block(hidden3, weights['conv4'], weights['b4'], is_training, reuse, scope + '3' + str(step))
        hidden4 = tf.reshape(hidden4, [-1, np.prod([int(dim) for dim in hidden4.get_shape()[1:]])])
